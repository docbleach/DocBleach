...
public class PdfBleach implements Bleach {
  ...
  private void sanitize(RandomAccessRead source, OutputStream outputStream, BleachSession session)
  ...
  private void sanitizeNamed(
    ...
    sanitizeRecursiveNameTree(
        names.getJavaScript(), action -> sanitizeJavascript(session, doc, action));
    names.setJavascript(null);
  }

  private void sanitizeJavascript(
      BleachSession session, PDDocument doc, PDActionJavaScript action) {
  ...
  private void sanitizeEmbeddedFile(
      BleachSession session, PDDocument doc, PDComplexFileSpecification fileSpec) {
    LOGGER.trace("Embedded file found: {}", fileSpec.getFilename());

    Function<PDEmbeddedFile, PDEmbeddedFile> sanitize =
        file -> sanitizeEmbeddedFile(session, doc, file);
  ...
  public void sanitizeOpenAction(BleachSession session, PDDocumentCatalog docCatalog)
      throws IOException {
  ...
  public void sanitizeDocumentActions(
      BleachSession session, PDDocumentCatalogAdditionalActions documentActions) {
  ...
  public void sanitizeFieldAdditionalActions(
      BleachSession session, PDFormFieldAdditionalActions fieldActions) {
    ...
    if (fieldActions.getK() != null) {
      ...
      recordJavascriptThreat(
          session, "FormAdditionalActions", "Action when the user types a keystoke");
    }
    if (fieldActions.getV() != null) {
      ...
      recordJavascriptThreat(
          session, "FormAdditionalActions", "Action when the field's value is changed");
    }
  }
  ...
  public void sanitizeAnnotationActions(
      BleachSession session, PDAnnotationAdditionalActions annotationAdditionalActions) {
    ...
    if (annotationAdditionalActions.getE() != null) {
      ...
      recordJavascriptThreat(
          session, "Annotation", "Action when the cursor enters the annotation's active area");
    }
    if (annotationAdditionalActions.getFo() != null) {
      ...
      recordJavascriptThreat(
          session, "Annotation", "Action when the annotation receives the input focus");
    }
    if (annotationAdditionalActions.getPC() != null) {
      ...
      recordJavascriptThreat(
          session, "Annotation", "Action when the page containing the annotation is closed");
    }
    ...
    if (annotationAdditionalActions.getPO() != null) {
      ...
      recordJavascriptThreat(
          session, "Annotation", "Action when the page containing the annotation is opened");
    }
    if (annotationAdditionalActions.getPV() != null) {
      ...
      recordJavascriptThreat(
          session, "Annotation", "Action the page containing the annotation becomes visible");
    }
    ...
    if (annotationAdditionalActions.getX() != null) {
      ...
      recordJavascriptThreat(
          session, "Annotation", "Action when the cursor exits the annotation's active area");
    }
  }
